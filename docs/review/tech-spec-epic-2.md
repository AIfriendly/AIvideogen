...
interface QualityIssue {
  type: 'low_information_density' | 'filler_language' | 'vague_content' | 'tts_formatting';
...
// - low_information_density: Less than 3 factual elements per 100 words
// - filler_language: Contains subjective adjectives, hedging words ("obviously", "incredibly", "basically")
// - vague_content: Uses generalizations without concrete specifics
// - tts_formatting: Contains markdown, meta-labels, or non-speakable characters
```
...
2. **Script Generation Phase:**
   ...
   - Analyze topic for appropriate delivery style (gaming analysis, historical events, technical explanations)
   - Generate prompt using script-generation-prompt.ts template (informational/scientific delivery)
   - Call LLM (Ollama/Llama 3.2 or Gemini 2.5) via provider abstraction
   - Parse JSON response into scene array
   - Run informational quality validation (validate-script-quality.ts):
     - Check information density (≥3 factual elements per 100 words)
     - Detect filler language (hedging words, subjective adjectives)
     - Reject vague/unfocused content
     - Verify TTS readiness (no markdown, meta-labels)
   - If validation fails: Retry with improved prompt (max 6 attempts)
...
- **Retry Logic:** Automatic retry for LLM failures (max 6 attempts - 3 for quality issues with progressive prompt enhancement, 3 for technical failures with exponential backoff)
...
**AC4: Script Quality Validation (Informational Standards)**
- Given: A script is generated by the LLM
- When: Informational quality validation runs
- Then: Scripts must be validated against informational quality standards:
  - Information density: ≥3 factual elements per 100 words
  - No filler language: Reject hedging words ("obviously", "incredibly", "basically", "kind of")
  - No vagueness: Reject generalizations without concrete specifics (e.g., "many bosses" without names)
  - TTS readiness: No markdown, meta-labels, or formatting characters
- And: Vague, unfocused, or filler-heavy scripts must be rejected
- And: System retries with improved prompts (max 6 attempts total)
- And: Only scripts meeting informational quality standards proceed to TTS
...
| AC3: Informational Scripts | Workflows: Script Generation Phase | lib/llm/prompts/script-generation-prompt.ts | Unit test: Validate informational delivery rules |
| AC4: Quality Validation | APIs: ScriptQualityValidator | lib/llm/validate-script-quality.ts | Unit test: Test rejection of vague/filler scripts |
...
- Script information density verification (≥3 factual elements per 100 words)
- Verify no filler language in generated scripts
- Verify topic-appropriate delivery (gaming: mechanics/strategies, historical: dates/events, technical: step-by-step)
